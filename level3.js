...
class CloudStorage extends CloudStorageInterface {
  ...
  compressFile(userId, name) {
    if (!this._isValidName(name) || name.endsWith(COMPRESSED_SUFFIX)) {
      return null;
    }
    ...
    const compressedName = `${name}${COMPRESSED_SUFFIX}`;
    if (this._hasFile(compressedName)) {
      return null;
    }

    const compressedSize = record.size / 2;
    if (!Number.isInteger(compressedSize)) {
      return null;
    }

    this._removeFile(name);
    this._recordFile(compressedName, compressedSize, userId);
